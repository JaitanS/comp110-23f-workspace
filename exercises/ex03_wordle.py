...
def input_guess(length_secret_word: int) -> str: 
    """Ensures the length of the guess is valid."""
    guess = input(f"Enter a {length_secret_word} character word: ")
    while len(guess) != length_secret_word: 
        guess = input(f"That wasn't {length_secret_word} chars! Try again: ")
    return guess
...
def main() -> None: 
    ...
    while (turn_count + 1) < 7 and win < 1: 
        turn_count += 1
